<#
    ...
    1.5:	
            ...
            - SSL certificate errors can be ignored now
    ...
    1.6:
            - fixed bug (adding delegates when delegatelist is empty)
...
function Remove-Delegate {
    ...
    try {
        ...
        for ($i = 0; $i -le $DelegateList.Count - 1; $i++) {
            $logger.Write("$($DelegateList[$i]) removed from $($Identity) successfully.", 1, $true)
...
function Set-Delegate {
    ...
    for ($i = 0; $i -le $Delegates.Count - 1; $i++) {
        $logger.Write("Set the delegate $($Delegates[$i]) to mailbox with following permissions:", 0, $WriteOnConsole)
...
if ($service -ne $null) {
    # first check, if DelegateToRemove or DelegateToSet got any entries
    if ($DelegateToRemove.Count -eq 0 -or $DelegateToSet.Count -eq 0) {
        $DelegateList = $null
        $DelegateList = Get-Delegates -Service $service -Identity $Identity
    
        if ($DelegateList -ne $null) {
            if ($DelegateList.DelegateUserResponses.Count -ge 1) {
            ...
            $logger.Write("Failed to get delegates for $($Idenitty). Check log file for errors.", 0, $WriteOnConsole)
        }   
    }
    ...
    if ($DelegateToRemove.Count -ge 1 -or $DelegateToSet.Count -ge 1) {
        ...
        if ($DelegateToRemove.Count -ge 1) {
            for ($i = 0; $i -le $DelegateToRemove.Count - 1; $i++) {
                if ($FoundDelegates.ContainsValue($DelegateToRemove[$i].ToLower())) {
                    if ($Confirm) {
                        if ((Request-Choice -Caption "$($DelegateToRemove[$i]) found. Do you want to remove the delegate?") -eq 0) {
                            $ToRemove += $DelegateToRemove[$i].ToLower()
        ...
        if ($DelegateToSet.Count -ge 1) {
            If ($FoundDelegates.Count -ge 1) {
                foreach ($User in $DelegateToSet) {
                    if ($FoundDelegates.ContainsValue($User.ToLower())) {
                        # User is delegate, we need to alter him
                        $DelegateToAlterList += $User
                    }
                    else {
                        # New delegate to add
                        $DelegateAddList += $User
                    }
                }
            }
            else {
